...
{
    [HttpPost("retrospectives/{retrospectiveId}/items")]
    ...
    [HttpPatch("{id:int}")]
    [Authorize(Policy = "ParticipantOrOwner")]
    public async Task<IActionResult> Update(int id, [FromBody] PatchGroupItemRequest groupItem,
        CancellationToken cancellationToken)
    {
    ...
    [HttpPut("{id:int}/move")]
    [Authorize(Policy = "ParticipantOrOwner")]
    public async Task<IActionResult> Move(int id, [FromBody] MoveGroupItemRequest request,
        CancellationToken cancellationToken)
    {
    ...
    {
        await service.DeleteGroupItemAsync(id, UserId, cancellationToken);
        return NoContent();
    }
